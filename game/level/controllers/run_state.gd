...
signal collect_and_quit_requested
...
func _ready() -> void:
	...
	pause_menu.collect_quit_pressed.connect(_on_collect_quit)
...
func _on_collect_quit() -> void:
	if get_tree().paused:
		get_tree().paused = false
	emit_signal("collect_and_quit_requested")
...
func update_hud() -> void:
	...
	health_value_label.text = str(run_root.health_controller.get_current_health())
